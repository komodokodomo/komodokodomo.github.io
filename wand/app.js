...
function registerSVG(){
    ...
    // camera.body = Snap.select("#camera-filler");
    ...
    camera.discard.addClass("allow-pointer");
    camera.flip.addClass("allow-pointer");
    camera.snap.addClass("allow-pointer");
...
function preventZoom(){
    let rb = document.getElementById("right-button");
    rb.addEventListener("touchend", ev => {
        console.log("rb touched");
        ev.preventDefault();
        rb.click();
      }, true);

    let lb = document.getElementById("left-button");
    lb.addEventListener("touchend", ev => {
        console.log("lb touched");
        ev.preventDefault();
        lb.click();
        }, true);
}
...
function showRelevantSVG(){
    ...
    DOM_EL.activityShapesContainer.addClass("hide");
    setTimeout(()=>{
        DOM_EL.activityPaletteContainer.hide();
        DOM_EL.activitySpeechContainer.hide();
        ...
        if(APP_STATE.mode == 0){
            ...
            CNV_EL.bubble.textDiv.hide();
            DOM_EL.activityShapesContainer.style("display","flex");
            DOM_EL.activityShapesContainer.removeClass("hide");
            DOM_EL.capture.hide();
            retractButtons();
        }
    },300);
...
function resetLoginIframe() {
    ...
    if(IFRAME.loginframeLoaded){
        ...
        const form = IFRAME.loginframe.contentDocument.body.querySelectorAll("form")[0];
        nicknameInput.blur();
        passwordInput.blur();
        form.blur();
        loginBox.removeChild(IFRAME.loginframe);
        IFRAME.loginframeLoaded = false;
    }
    else{
