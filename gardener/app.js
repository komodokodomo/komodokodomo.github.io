  ...
  if(result)
      {
        if(label !== "irrelevant"){
          if(result[0].confidence > PREDICT_LOWERBOUND && result[0].confidence < PREDICT_UPPERBOUND && APP_STATE.evidenceFound == false){
            ...
            DOM_EL.personaButton.html("📸 document findings"); //📸 capture evidence
          }
            ...
            if(APP_STATE.evidencesFound.includes(APP_STATE.evidenceDetected)){
              DOM_EL.personaButton.html("📸 redocument findings"); //📸 capture evidence
            }
            else{
            }
          }
...
function contentCloseEvent(){
  ...
  setTimeout(function(){
    ...
    if(APP_STATE.evidencesFound.length == APP_STATE.numClasses && APP_STATE.completed == false){
      ...
      DOM_EL.evidenceHeader.html("All Plants Spotted!");
    }
  },300);
}
...
function captureEvidenceEvent(){
  if(DOM_EL.personaButton.class() !== "inactive"){
    ...
    if(APP_STATE.evidencesFound.includes(APP_STATE.evidenceDetected)){
      ...
      DOM_EL.evidenceSubheader.html("Click on the documented plants to see what can be done with it!");
      
      APP_STATE.evidencesFound[APP_STATE.evidenceCounter] = APP_STATE.evidenceDetected;
      APP_STATE.evidenceCounter++;

      DOM_EL.evidenceHeader.html(APP_STATE.evidenceCounter.toString()+ "/" + APP_STATE.numClasses + " Plants Spotted");

      
      DOM_EL.contentHeader.html("Findings " +  DOM_EL.evidenceListItemContainer[APP_STATE.evidenceCounter-1].attribute("index") + " : " + DOM_EL.evidenceListItemTitle[APP_STATE.evidenceCounter-1].html());
...
function setup(){
  ...
  UTIL.descriptionQuill = new Quill('#onboard-content', {
    ...
    placeholder: 'No background was given to you, look around the garden to see what you can learn about it!',
    theme: 'snow'  // or 'bubble'
  });
...
function gotNumClasses(err,result){
  ...
  if(err){
    ...
    for(let i = 0; i < APP_STATE.numClasses; i++){
      ...
      DOM_EL.evidenceListItem[i].mousePressed(function(){
        if(DOM_EL.evidenceListItem[i].class().includes("noimage")){
          ...
          DOM_EL.contentHeader.html("Findings " +  DOM_EL.evidenceListItemContainer[i].attribute("index") + " : " + DOM_EL.evidenceListItemTitle[i].html());
    ...
    DOM_EL.evidenceHeader.html("0/" + APP_STATE.numClasses + " Findings Documented");
