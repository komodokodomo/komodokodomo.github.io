...
function onSignIn(googleUser) {
  ...
  DOM_EL.roomsTitle.html("Signed in as " + APP_STATE.username);

  let u = "?account=" + APP_STATE.username;
  var xhr = new XMLHttpRequest();
  xhr.open('GET', '/admin/GET_BW_PROFILE' + u, true);
  xhr.onload = function(e) {
      if (this.status == 200) {
        ...
        Object.keys(data).forEach((key)=>{
          DOM_EL.roomContainers[key] = new RoomContainer(data[key].title,data[key].pin);
        });
      }
      else if(this.status == 404 || this.status == 504) {
          console.log(`server failed received request to get ${profile}'s`);
      }
      DOM_EL.loginContainer.hide();
      DOM_EL.roomsContainer.style("display","flex");
      if(Object.keys(DOM_EL.roomContainers).length > 0){
        DOM_EL.addRoomContainer.removeClass("initial");
      }
    };
  xhr.send();
}
...
var UTIL = {
    ...
    index: 0,
    pdf: null,
    pauseEmitInterval: null,
    playEmitInterval: null
}
...
function playEmitTimer() {
  console.log( APP_STATE.playEmitCounter );
  UTIL.socket.emit("bubblepit_all_play",{
    "room" : APP_STATE.room,
    "user" : APP_STATE.username
  });
  if ( APP_STATE.playEmitCounter < 1 ) {
      console.log( 'Reaching Stop' ); 
      clearInterval( UTIL.playEmitInterval );
      return;         
  } 
  APP_STATE.playEmitCounter -= 1;
}
function pauseEmitTimer() {
    console.log( APP_STATE.pauseEmitCounter );
    UTIL.socket.emit("bubblepit_all_pause",{
      "room" : APP_STATE.room,
      "user" : APP_STATE.username
    });
    if ( APP_STATE.pauseEmitCounter < 1 ) {
        console.log( 'Reaching Stop' ); 
        clearInterval( UTIL.pauseEmitInterval );
        return;         
    } 
    APP_STATE.pauseEmitCounter -= 1;
}
...
function updateRoomQuestion(pin,key,question){
  var xhr = new XMLHttpRequest();
  let u = "&user=" + APP_STATE.username
  let q = "&question=" + question;
  let k = "&key=" + key; 
 
  xhr.open('GET', '/admin/update_room?room=' + pin + u + k + q, true);
...
function updatePraiseList(pin,key,id,add=0){
  ...
  xhr.open('GET', '/admin/update_praise_list?room=' + pin + k + i + a + u, true);
...
function clearRoomResponse(pin,key,name,id){
  ...
  xhr.open('GET', '/admin/clear_room_response?room=' + pin + k + n + i + u, true);
...
function clearRoomResponses(user,pin,key){
  var xhr = new XMLHttpRequest();
  let k = "&key=" + key; 
  let u = "&user=" + APP_STATE.username; 

  xhr.open('GET', '/admin/clear_room_responses?room=' + pin + k + u, true);
...
function createDuplicateRoom(pin,username,title,reference){
  let user = "&user=" + username;
  let t = "&title=" + title;
  let r = "&reference=" + reference;
  var xhr = new XMLHttpRequest();
  xhr.open('GET', '/admin/bubblepit_create_duplicate_room?pin=' + pin + user + t + r, true);

  xhr.onload = function(e) {
      if (this.status == 200) {
        console.log(`SUCCESS : duplicated room ${reference} with new ${pin} belonging to user: ${username}`);
      }
      else if(this.status == 404) {
        console.log(`FAILED : duplicated room ${reference} belonging to user: ${username}`);
      }
    };
  xhr.send("");
}
function createRoom(pin,username,title){
  let user = "&user=" + username;
  let t = "&title=" + title;
  var xhr = new XMLHttpRequest();
  xhr.open('GET', '/admin/bubblepit_create_room?pin=' + pin + user + t, true);

  xhr.onload = function(e) {
      if (this.status == 200) {
        console.log(`SUCCESS : create room ${pin} belonging to user: ${username}`);
      }
      else if(this.status == 404) {
        console.log(`FAILED : create room ${pin} belonging to user: ${username}`);
      }
    };
  xhr.send("");
}

function deleteRoom(pin,username){
  let user = "&user=" + username;
  var xhr = new XMLHttpRequest();
  xhr.open('GET', '/admin/bubblepit_delete_room?pin=' + pin + user, true);

  xhr.onload = function(e) {
      if (this.status == 200) {
        console.log(`SUCCESS : delete room ${pin} belonging to user: ${username}`);
      }
      else if(this.status == 404) {
        console.log(`FAILED : delete room ${pin} belonging to user: ${username}`);
      }
    };
  xhr.send("");
}

async function getRoom(user,pin){
      await fetch('/admin/bubblepit_get_room?pin=' + pin + "&user=" + user)
      .then(
        function(response) {
          if (response.status == 200) {
            response.json().then(function(data) {
              ...
              UTIL.socket.emit('bubblepit_login',{
                "room" : pin,
                "user" : APP_STATE.username
              });
              APP_STATE.room = pin;
              DOM_EL.lobbyRoom.html("room " + pin);
              DOM_EL.activityInstructionsRoom.html("room " + pin);
              DOM_EL.roomsContainer.hide();
              DOM_EL.loginContainer.hide();
...
function enterMainEvent(){
  DOM_EL.lobbyContainer.hide();
  DOM_EL.mainContainer.show();
  if(Object.keys(DOM_EL.questionContainers).length>0){
    DOM_EL.questionContainers[Object.keys(DOM_EL.questionContainers)[0]].loadQuestion();
    windowResized();
  }
  updateUserCount();
}
...
function startCon()
{
  ...
  UTIL.socket.on('bubble-login-event', function(msg) 
  {
    if(msg.room == APP_STATE.room){
      if(APP_STATE.pause){
        UTIL.socket.emit("bubblepit_all_pause",{"room" : APP_STATE.room, "user" : APP_STATE.username});
      }
    
      if(APP_STATE.question !== null){
        UTIL.socket.emit("bubblepit_question", {
          "room" : APP_STATE.room, 
          "value" : APP_STATE.question, 
          "type": APP_STATE.questionType,
          "user" : APP_STATE.username
        });
      }
...
var DOM_EL = {
  ...
  roomsContainer: null,
    roomsInnerContainer: null,
      roomContainers: {},
    roomsTitle: null,
    addRoomContainer: null,
    roomsLogout : null,
...
function setup(){
    ...
    DOM_EL.loginContainer = select("#login-container");
      ...
      DOM_EL.loginQuickStart = select("#options-buzz");
      DOM_EL.loginQuickStart.mousePressed(quickStartEvent);
    ...
    DOM_EL.lobbyButton = select("#lobby-button");
      DOM_EL.lobbyButton.mousePressed(enterMainEvent);

    DOM_EL.roomsContainer = select("#rooms-container");
    DOM_EL.roomsInnerContainer = select("#rooms-inner-container");
    DOM_EL.roomsTitle = select("#rooms-title");
    DOM_EL.roomsLogout = select("#rooms-title-button-logout");
    DOM_EL.roomsLogout.mousePressed(logoutEvent);
    DOM_EL.addRoomContainer = select("#add-room-container");
    DOM_EL.addRoomContainer.mousePressed(addRoomEvent);
    DOM_EL.roomsContainer.hide();
    ...
    DOM_EL.mainContainer = select("#main-container");
      ...
      DOM_EL.questionContainer.mouseOver(()=>{DOM_EL.questionContainer.removeClass("hidden");});
      DOM_EL.questionContainer.mouseOut(()=>{DOM_EL.questionContainer.addClass("hidden");});
    ...
    DOM_EL.userContainer.mouseOver(()=>{DOM_EL.userContainer.removeClass("hidden");});
    DOM_EL.userContainer.mouseOut(()=>{DOM_EL.userContainer.addClass("hidden");});
    ...
    // DOM_EL.quillContainer.id("quill-container");

    QUILL.editor = new Quill('#quill-editor-container', {
      modules: {
    ...
    QUILL.editor.on('text-change', function() {
      var justHtml = QUILL.editor.root.innerHTML;
      CNV_EL.balls[APP_STATE.activeBall].contentDivChild.elt.innerHTML = justHtml;
    });

    DOM_EL.quillContainer.hide();
...
async function quickStartEvent(){
  let num = await getRoomPin();
  if(num !== null){
    APP_STATE.username = "quickstart";
    APP_STATE.room = num;
    createRoom(num,APP_STATE.username,"quickstart")
    windowResized();
    UTIL.socket.emit('bubblepit_login',{
      "room" : num,
      "user" : "quickstart"
    });
    DOM_EL.lobbyRoom.html("room " + num);
    DOM_EL.activityInstructionsRoom.html("room " + num);
    DOM_EL.roomsContainer.hide();
    DOM_EL.loginContainer.hide();
    DOM_EL.lobbyContainer.show();
    updateUserCount();
...
async function getRoomPin(){
  let pin = "";
  for(let i = 0; i<4; i++){
    pin += round(random(0,9)).toString();
  }
  // console.log(pin);
  await fetch('/admin/bubblepit_get_roompin?pin=' + pin + "&user=" + APP_STATE.username)
  .then(
    function(response) {
      if (response.status == 200) {
        // return pin;
  ...
  // let p = "&pin=" + pin;
  // var xhr = new XMLHttpRequest();
  // xhr.open('GET', '/admin/bubblepit_get_roompin' + u + p, true);
  // xhr.onload = function(e) {
  //     if (this.status == 200) {
  //       console.log(this.response);
  //       return pin;
  //     }
  //     else if(this.status == 404 || this.status == 504) {
  //         console.log(`server failed received request to get randomized room pin`);
  //         return null;
  //     }
  //   };
  // xhr.send();
...
async function addRoomEvent(){
  DOM_EL.addRoomContainer.removeClass("initial");
  if(checkNumRooms() == true){
  let num = await getRoomPin();
  // console.log(num);
    if(num !== null){
      DOM_EL.roomContainers[num] =  new RoomContainer("click to edit",num);
      createRoom(num,APP_STATE.username,"click to edit");
    }else{
      addRoomEvent();
    }
  }
...
function checkNumRooms(){
  if(Object.keys(DOM_EL.roomContainers).length<3){
    return true;
  }else{
    return false;
  }
...
function logoutEvent(){
  ...
  auth2.signOut().then(function () {
    console.log('User signed out.');
    DOM_EL.roomsContainer.hide();
    DOM_EL.loginContainer.style("display","flex");
  });
}
...
function triggerFeedback(){
  UTIL.socket.emit("bubblepit_feedback",{
    "room":APP_STATE.room,
    "user" : APP_STATE.username
  });
}
...
function createNewActivity(){
  if(DOM_EL.newActivityInput.value().length > 0){
    if(APP_STATE.activityOptionTypeChosen !== null){
      ...
      DOM_EL.questionContainers[n].loadQuestion();
      UTIL.socket.emit("bubblepit_question", {
        "room" : APP_STATE.room, 
        "value" : DOM_EL.questionContainers[n].questionListContent.html(),
        "type": DOM_EL.questionContainers[n].type,
        "user" : APP_STATE.username
      });
      windowResized();
...
function newQuestionCache(key,title,type,anon, categorize, limited){
  ...
  var xhr = new XMLHttpRequest();
  xhr.open('GET', '/admin/create_room_cache' + p + u + k + t + q + a + c + l, true);
  xhr.onload = function () {
...
function addQuestionEvent(){
  ...
  if(APP_STATE.activeBall!==null && APP_STATE.activeBall!==undefined){
    CNV_EL.balls[APP_STATE.activeBall].stopDisplayMain();
    console.log("stopping display of particular ball");
  }
  else{
    console.log("no ball selected, just add question");
  }
}
...
function pauseEvent(){
  if(DOM_EL.activityPause.class().includes("active")){
    ...
    DOM_EL.activityPause.removeClass("active");
    UTIL.playEmitInterval = setInterval( playEmitTimer, 1000 );
    if ( APP_STATE.pauseEmitCounter > 0 ) {
      console.log("cancelling pauseEmitInterval");
      clearInterval( UTIL.pauseEmitInterval );
      APP_STATE.pauseEmitCounter = 5;
    } 
    APP_STATE.pause = false;
  }
  else{
    ...
    DOM_EL.activityPause.addClass("active");
    if ( APP_STATE.playEmitCounter > 0 ) {
      console.log("cancelling playEmitInterval");
      clearInterval( UTIL.playEmitInterval );
      APP_STATE.playEmitCounter = 5;
    } 
    UTIL.pauseEmitInterval = setInterval( pauseEmitTimer, 1000 );
    APP_STATE.pause = true;
  }
}
...
function globalStopDisplayMain(){
  ...
  CNV_EL.balls.forEach(ball => {
    ...
    ball.editDiv.removeClass("expanded");
    DOM_EL.quillContainer.hide();
  });
}
...
class Ball {
  constructor(id, title,contents,color,xin, yin, din, idin, oin) {
    ...
    this.editDiv.parent(this.div);
    this.editDiv.addClass("bubble-edit");
    this.editDiv.addClass("hidden");
    this.editDiv.mousePressed(this.editEvent.bind(this));
  ...
  stopDisplayMain(){
    ...
    if(this.editing){
      if(!this.imageBall){
        this.contentDivChild.show();
        this.quillDelta = QUILL.editor.getContents();
        DOM_EL.quillContainer.hide();
      }
      this.editing = false;
    }
    ...
    //   this.quillDelta = QUILL.editor.getContents();
    //   DOM_EL.quillContainer.hide();
    //   if(!this.imageBall){this.contentDivChild.show();}
    // },20);
    this.closeContentDiv.addClass("hide");
    ...
    this.editDiv.removeClass("expanded");
    APP_STATE.activeBall = null;
  }
  ...
  displayContentDiv(){
    ...
    this.editDiv.addClass("expanded");
    APP_STATE.activeBall = this.id;
    APP_STATE.pauseAnimation = true;
    this.displayMain = true;
    if(!this.imageBall){
      this.editDiv.removeClass("hidden");
    }
  }
  ...
  showOptions(){
    ...
    if(!this.imageBall && this.displayMain){
      this.editDiv.removeClass("hidden");
    }
  }
  ...
  closeBubble(){ 
    ...
    this.mouseOutBubble();
    DOM_EL.quillContainer.hide();
    DOM_EL.quillContainer.parent(DOM_EL.activityContentContainer);
  ...
  editEvent(){
    this.editing = true;
    this.contentDivChild.hide();
    DOM_EL.quillContainer.parent(this.contentDiv);
    if(this.quillDelta!== null){
      QUILL.editor.setContents(this.quillDelta);
    }else{
      QUILL.editor.setText(this.contentDivChild.html());
    }
    DOM_EL.quillContainer.show();
  }
...
class QuestionContainer {
 ...
 questionClicked(){
  ...
  UTIL.socket.emit("bubblepit_question", {
    "room" : APP_STATE.room, 
    "value" : this.questionListContent.html(),
    "type": this.type,
    "user" : APP_STATE.username
  });
 ...
 removeQuestion(){
  if (confirm('Are you sure you want to delete this question? Changes are permanent')) {
    ...
    let r = "?room=" + APP_STATE.room;
    let u = "&user=" + APP_STATE.username;
    let k = "&key=" + this.id;

    var xhr = new XMLHttpRequest();
    xhr.open('GET', '/admin/delete_room_question' + r + u + k, true);
    xhr.onload = function () {
...
function editRoomTitle(user,room,title){
  let r = "&pin=" + room;
  let t = "&title=" + title;

  var xhr = new XMLHttpRequest();
  xhr.open('GET', '/admin/bubblepit_edit_room_title?user=' + user + r + t, true);
  xhr.onload = function(e) {
      if (this.status == 200) {
        console.log("title edited")
      }
      else if(this.status == 404 || this.status == 504) {
          console.log(`server failed received request to edit title`);
      }
    };
  xhr.send();
}
...
class RoomContainer{
  constructor(title,pin){
    ...
    this.container.parent(DOM_EL.roomsInnerContainer);
    this.container.addClass("room-container");

      this.optionsButtonDiv = createDiv("…");
      this.optionsButtonDiv.parent(this.container);
      this.optionsButtonDiv.addClass("room-options-button");
      this.optionsButtonDiv.mousePressed(this.openOptions.bind(this))

      this.titleContainer = createDiv();
      this.titleContainer.parent(this.container);
      this.titleContainer.addClass("room-title-container");
      this.titleContainer.mouseOver(this.showTitleEdit.bind(this));
      this.titleContainer.mouseOut(this.hideTitleEdit.bind(this));
      
        this.title = createDiv(title);
        this.title.parent(this.titleContainer);
        this.title.addClass("room-title");
        this.title.mousePressed(this.editTitle.bind(this));

        this.titleEditIcon = createDiv("✏️");
        this.titleEditIcon.parent(this.titleContainer);
        this.titleEditIcon.addClass("room-title-icon");
        this.titleEditIcon.hide();

        this.titleInput = createInput();
        this.titleInput.parent(this.titleContainer);
        this.titleInput.addClass("room-title-input");
        this.titleInput.hide();
        this.titleInput.elt.onblur = this.changeTitle.bind(this);
  
      this.pinDiv = createDiv(pin);
      this.pinDiv.parent(this.container);
      this.pinDiv.addClass("room-pin");

      this.openDiv = createDiv("Open");
      this.openDiv.parent(this.container);
      this.openDiv.addClass("room-button");
      this.openDiv.mousePressed(this.openRoom.bind(this));

      this.optionsDiv = createDiv();
      this.optionsDiv.parent(this.container);
      this.optionsDiv.addClass("room-options");
      this.optionsDiv.addClass("hidden");
      
        this.optionsDuplicateDiv = createDiv("Duplicate");
        this.optionsDuplicateDiv.parent(this.optionsDiv);
        this.optionsDuplicateDiv.addClass("room-option");
        this.optionsDuplicateDiv.addClass("duplicate");
        this.optionsDuplicateDiv.mousePressed(this.duplicateRoom.bind(this));

        this.optionsRemoveDiv = createDiv("Remove");
        this.optionsRemoveDiv.parent(this.optionsDiv);
        this.optionsRemoveDiv.addClass("room-option");
        this.optionsRemoveDiv.addClass("remove");
        this.optionsRemoveDiv.mousePressed(this.removeRoom.bind(this));
  }
  changeTitle(){
    this.title.html(this.titleInput.value());
    this.title.show();
    this.titleEditIcon.show();
    this.titleInput.hide();
    this.editing = false;
    this.titleEditIcon.hide();
    editRoomTitle(APP_STATE.username,this.pinDiv.html(),this.title.html());
  }
  showTitleEdit(){
    if(!this.editing){
      this.titleEditIcon.show();
  ...
  editTitle(){
    ...
    setTimeout(()=>{
      this.title.hide();
      this.titleEditIcon.hide();
      this.titleInput.value(this.title.html());
  ...
  openOptions(){
    this.optionsDiv.toggleClass("hidden");
  }
  openRoom(){
    getRoom(APP_STATE.username,this.pinDiv.html());
  }
  async duplicateRoom(){
    if(checkNumRooms() == true){
      let num = await getRoomPin();
      DOM_EL.roomContainers[num] = new RoomContainer("copy of " + this.title.html(),num);
      createDuplicateRoom(num,APP_STATE.username,"copy of " + this.title.html(),this.pinDiv.html());
      this.openOptions();
    }
    else{
      alert("too many rooms");
  ...
  removeRoom(){
    let pin = this.pinDiv.html();
    deleteRoom(pin,APP_STATE.username);
    this.container.remove();
    delete DOM_EL.roomContainers[pin];
    if(Object.keys(DOM_EL.roomContainers).length == 0){
      DOM_EL.addRoomContainer.addClass("initial");
    }
  }
}
